#!/usr/bin/php
<?php
    function get_src_code($web_name) {
        $site = file_get_contents("https://".$web_name);
        preg_match("/<body[^>]*>(.*?)<\/body>/is", $site, $haystack);
        return($haystack[1]);
    }
    ...
    function get_images($haystack) {
        preg_match_all('/src\s*=\s*"(.+?)"/', $haystack, $images);
        return ($images);
    }
    ...
    $haystack = get_src_code($argv[1]);
    $img_links = get_images($haystack);
    $i = 0;
    $pos = 0;
    $links = $img_links[1];
    $img = $img_links[0][$pos];
